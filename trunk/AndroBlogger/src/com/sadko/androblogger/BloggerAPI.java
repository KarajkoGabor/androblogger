...
public class BloggerAPI
	...
	/**
	 * The only data we need from this config editor is the post link URL of the
	 * blog that user has selected in the spinner.
	 */

	public CharSequence getConfigEditorData() {
		if (currentySelectedBlog == -1) {
			Alert
					.showAlert(parentRef, "Select a blog",
							"You need to select one blog prior saving the configuration.");
			return "";
		} else {
			if (currentySelectedBlog > blogIds.length - 1) {
				Log
						.e(TAG,
								"System allows spinner index to be selected outside source data!");
				return "";
			}
	...
	public boolean createPost(Activity parent, String authToken,
		...
		if (myService == null) {
			...
			Log
					.d(
							TAG,
							"GoogleService is null while creating post. Are you calling this from outside the PreviewDialog? ");
		}
		...
		if (myService == null) {
			Log
					.e(
							TAG,
							"The GoogleService is null while creating post. Are you calling this from outside the PreviewDialog?");
			return false;
		}
		try {
	...
	public Feed getAllPosts(String username, String password)
		...
		/*
		 * for (int i = 0; i < resultFeed.getEntries().size(); i++) { Entry
	...
	public String getPostUrl(String authToken) throws ServiceException,
			IOException {
		String blogId = getBlogId(myService);
	...
	private void getUserBlogs() {
		if (parentRef == null) {
			Log
					.e(
							TAG,
							"Error: Can't get the parent blogs without a valid reference to invoking activity!");
		...
		fetchProgress = ProgressDialog.show(parentRef, "Verifying the data",
		...
		Thread fetchGoogleBlogs = new Thread() {
			public void run() {
				...
				Log.d(TAG, "Querying Blogger blog, URL:" + feedUrl + ", user: "
						+ fetchUsername + ", pass:" + fetchPassword);
				GoogleService myFetchService = new GoogleService("blogger",
						BlogConfigBLOGGER.APPNAME);
				statusMsg = mHandler.obtainMessage();
				status.putString(MSG_KEY, "2");
				statusMsg.setData(status);
				mHandler.sendMessage(statusMsg);
				try {
					myFetchService.setUserCredentials(fetchUsername,
							fetchPassword);
				} catch (com.google.gdata.util.AuthenticationException e) {
					Log.e(TAG, "Authentication exception! " + e.getMessage());
					statusMsg = mHandler.obtainMessage();
				...
				for (int i = 0; i < resultFeed.getEntries().size(); i++) {
					...
					while (iter.hasNext()) {
						...
						Log.d(TAG, "<link rel ='" + rel + "' type = '" + type
								+ "' href = '" + href + "'/>");
	...
	private void showFetchStatus() {
		...
		if (fetchStatus == 5) {
			...
			// disable the username and password so that user can't change them
			// anymore
			// since they are only valid for fetched blogs...
			EditText username = (EditText) parentRef
					.findViewById(R.id.Username);
			EditText password = (EditText) parentRef
					.findViewById(R.id.Password);
			Button fetchButton = (Button) parentRef
			...
			Alert.showAlert(parentRef, "Status",
	...
	private void updateSpinner(Spinner list) {
		if (blogNames != null) {
			...
			Object res = new ArrayAdapter(parentRef,
	...
	public void setInstanceConfig(CharSequence config) {
		if (config != null) {
			...
			Log
					.e(TAG,
							"Trying to set a null installce config when configuring Google Blogger API!");
		}
